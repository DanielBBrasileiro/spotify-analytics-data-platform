...
## Status
Accepted (Updated for Glue 5.1 & 2026 API Schemas)
...
## Decision
We decide to establish a **strict division of responsibilities** between **AWS Glue 5.1 / Apache Spark** (Data Lake tier) and **dbt Core** (Data Warehouse tier).

```
Raw JSON (Bronze S3)
       │
       ▼ [AWS Glue 5.1 / PySpark] -> Technical ETL & Serialization
...
### AWS Glue 5.1 / PySpark Responsibilities (Data Lake Tier):
...
3. **Item Validation & Unnesting**: Validate source playlist items (filtering unsupported non-track items) and explode nested arrays (`track.artists`, album references) using Spark distributed memory.
4. **Relational Extraction**: Normalize single JSON documents into discrete tabular entity datasets (`artists`, `albums`, `tracks`, `track_artists`, `playlist_snapshots`) along with source lineage (`spotify_snapshot_id`).
...
4. **Business Metrics & Marts**: Construct aggregated reporting tables (`mart_artist_presence`, `mart_playlist_trends`, `mart_track_lifecycle`, `mart_playlist_changes`).
...
| Dimension | AWS Glue 5.1 / PySpark | dbt Core on Snowflake |
| :--- | :--- | :--- |
| **Primary Domain** | Data Lake / Storage Optimization | Data Warehouse / Analytics Engineering |
| **Compute Paradigm** | Distributed Spark 3.5.6 JVM Memory | Massively Parallel SQL Engine (Snowflake) |
