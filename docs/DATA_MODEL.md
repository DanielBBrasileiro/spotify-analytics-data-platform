...
This document specifies planned schemas and models for the synthetic portfolio demonstration. No Silver, Snowflake, or dbt models are implemented yet. [ADR-0008](adr/0008-synthetic-analytics-and-source-use-boundary.md) defines the source-use boundary; the daily fact grain is unchanged.
...
Staging models are planned as views in dbt:
...
erDiagram
    ...
    dim_album {
        ...
        string album_id "Natural key: Spotify Album ID"
        string album_name
        string album_type
        date release_date
        int total_tracks
    ...
    fact_playlist_snapshot {
        string snapshot_pk PK "Surrogate Hash (playlist_id + snapshot_date + track_position)"
...
M5 must select one canonical observation per playlist/date, handle shorter or
empty replacement snapshots without stale fact slots, and test uniqueness of the
merge source. Duplicate track occurrences need an explicit mart aggregation rule.
The parser preserves null/duplicate artist billing slots; the planned non-null
bridge and pair deduplication require an explicit downstream policy before M3/M5.
These are implementation decisions, not new grain changes in this revision.

## 5. Layer 4: MARTS Schema (Analytical Business Marts)

Future Power BI reports will consume these models using synthetic data, labeled as simulated observations.
